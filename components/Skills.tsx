          ...
          <div key={category.title}>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-6">
              {category.skills.map((skill) => (
                <div key={skill.name} className="relative group flex flex-col items-center text-center p-4 rounded-sm bg-secondary/60 backdrop-blur-sm border border-dark-slate/50 shadow-md hover:shadow-lg hover:bg-secondary hover:-translate-y-1.5 transition-all duration-300 ease-custom-bezier">
                  <div className="w-12 h-12 mb-2 text-slate group-hover:text-accent-cyan transition-colors duration-300">{skill.icon}</div>
                  ...
                  {skill.description && (
                    <div className="absolute bottom-full mb-2 w-max max-w-xs p-3 text-sm text-left text-light-slate bg-secondary border border-dark-slate rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10">
                      <p className="font-bold text-accent-cyan -mt-1 mb-1">{skill.name}</p>
