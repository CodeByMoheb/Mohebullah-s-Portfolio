    ...
    <div className="grid grid-cols-1 md:grid-cols-5 gap-8 items-center mb-16">
      <div className={`relative group md:col-span-3 ${imageOrder}`}>
        <a href={project.liveUrl || project.githubUrl} target="_blank" rel="noopener noreferrer" className="block rounded-sm overflow-hidden">
          ...
          <img src={project.image} alt={project.title} className="rounded-sm w-full h-auto object-cover filter grayscale group-hover:grayscale-0 transition-all duration-300" />
        </a>
      </div>
...
const OtherProjectCard: React.FC<{ project: Project }> = ({ project }) => {
  return (
    <div className="bg-secondary p-6 rounded-sm shadow-lg hover:shadow-xl hover:-translate-y-2 transition-all duration-300 ease-custom-bezier flex flex-col group">
      ...
      <ul className="flex flex-wrap gap-x-4 text-xs text-slate font-mono">
        {project.tags.map(tag => <li key={tag}>{tag}</li>)}
      </ul>
    </div>
  )
